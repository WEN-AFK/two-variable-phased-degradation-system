...
lmada_1=sum(X_incresing1(j,1:10*changepoint(j)-1).*dt)/((10*changepoint(j)-1)*dt^2);
lmada_2=sum(X_incresing1(j,10*changepoint(j)+1:end).*dt)/sum(m*dt^2);
sigma_B_1=sum((X_incresing1(j,1:10*changepoint(j)-1)-lmada_1*dt).^2)/((10*changepoint(j)-1)*dt);
...
gamma_1 = mle(change_gamma1,'distribution','Normal');
tau_dis = mle(changepoint,'distribution','Normal');

parameter1(1,2)=lam_1(2);
parameter1(3,2)=lam_2(2);
parameter1=[parameter1;tau_dis;gamma_1];

save parameter1.mat parameter1
