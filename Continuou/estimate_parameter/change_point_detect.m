...
for i=1:m
miu_1=sum(X_incresing1(i,1:20))/20;
sigma_1=sqrt(sum((X_incresing1(i,1:20)-miu_1).^2)/20);

miu_2=sum(X_incresing2(i,1:20))/20;
sigma_2=sqrt(sum((X_incresing2(i,1:20)-miu_2).^2)/20);

change_point_detectiong=0;

for j=1:n
    if change_point_detectiong==0
        if abs(X_incresing1(i,j))>3*sigma_1 && abs(X_incresing2(i,j))>3*sigma_2
                miu_before1=sum(X_incresing1(i,j-19:j))/20;
                sigma_before1=sqrt(sum((X_incresing1(i,j-19:j)-miu_before1).^2)/20);
                miu_after1=sum(X_incresing1(i,j:j+19))/20;
                sigma_after1=sqrt(sum((X_incresing1(i,j:j+19)-miu_after1).^2)/20);
                
                miu_before2=sum(X_incresing2(i,j-19:j))/20;
                sigma_before2=sqrt(sum((X_incresing2(i,j-19:j)-miu_before2).^2)/20);
