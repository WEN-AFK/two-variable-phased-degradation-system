...
miu_1=sum(X_incresing1(dataset,1:20))/20;
sigma_1=sqrt(sum((X_incresing1(dataset,1:20)-miu_1).^2)/20);
miu_2=sum(X_incresing2(dataset,1:20))/20;
sigma_2=sqrt(sum((X_incresing2(dataset,1:20)-miu_2).^2)/20);

for hits=1:long

        if change_point_detectiong==0
            if abs(X_incresing1(dataset,hits))>3*sigma_1 && abs(X_incresing2(dataset,hits))>3*sigma_2
                    miu_before1=sum(X_incresing1(dataset,hits-19:hits))/20;
                    sigma_before1=sqrt(sum((X_incresing1(dataset,hits-19:hits)-miu_before1).^2)/20);
                    ...
                    miu_before2=sum(X_incresing2(dataset,hits-19:hits))/20;
                    sigma_before2=sqrt(sum((X_incresing2(dataset,hits-19:hits)-miu_before2).^2)/20);
...
xlabel('Cycles'); 
ylabel('Value'); 
line([1 changep_online/10],[0.01 0.01],'linestyle','-', 'Color','r', 'LineWidth', 1.5);
text(20,0.05,'Ture parameter')
...
xlabel('Cycles'); 
ylabel('Value');
line([1 changep_online/10],[0.07 0.07],'linestyle','-', 'Color','r', 'LineWidth', 1.5);
text(20,0.05,'Ture parameter')
...
Z_ture=X_incresing1(dataset,changep_online+1:end);
...
figure(4)
subplot(1,2,1)
plot(t(changep_online+1:end),lmada(1:end),'-*','LineWidth',3,'MarkerSize',6);
xlabel('Cycles'); 
ylabel('Value'); 
ylim([0,0.60]);
line([changep_online/10 100],[0.50 0.50],'linestyle','-', 'Color','r', 'LineWidth', 1.5);
text(70,0.5,'Ture parameter')
legend('Estimated {\lambda_{1,2}}'); 
set(gca,'FontSize',34,'FontWeight','bold' );
hold on

subplot(1,2,2)
plot(t(changep_online+1:end),sqrt(RR(1:end)),'-*','LineWidth',3,'MarkerSize',6);
xlabel('Cycles'); 
ylabel('Value'); 
line([changep_online/10 100],[0.40 0.40],'linestyle','-', 'Color','r', 'LineWidth', 1.5);
text(70,0.5,'Ture parameter')
